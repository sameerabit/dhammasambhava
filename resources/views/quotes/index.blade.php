    ...
    <div class="container mx-auto px-4">
        ...
        <div class="max-w-6xl mx-auto">
            <div class="grid md:grid-cols-2 gap-8">
                ...
                <div class="bg-white rounded-lg shadow-lg p-8 border-l-4 border-amber-600 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="mb-6">
                        <svg class="w-10 h-10 text-amber-300 mb-4" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z"/>
                        </svg>
                        <blockquote class="text-xl md:text-2xl text-gray-700 leading-relaxed font-serif italic">
                            "{{ $quote->text }}"
                        </blockquote>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                        <div>
                            <p class="text-lg font-bold text-amber-900">— {{ $quote->author }}</p>
                            @if($quote->source)
                            <p class="text-sm text-gray-600 mt-1">{{ $quote->source }}</p>
                            @endif
                        </div>
                        <a href="{{ route('quotes.show', $quote) }}"
                           class="text-amber-600 hover:text-amber-800 font-semibold transition flex items-center">
                            View
                    ...
                    @if($quote->category)
                    <div class="mt-4">
                        <span class="inline-block px-3 py-1 bg-amber-100 text-amber-800 rounded-full text-xs font-semibold uppercase">
                            {{ $quote->category }}
                        </span>
                    </div>
                    @endif
    ...
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto text-center">
            ...
            <a href="{{ route('quotes.show', $quoteOfTheDay) }}"
               class="inline-block mt-6 px-6 py-3 bg-white text-amber-600 rounded-lg hover:bg-gray-100 transition font-semibold">
                Explore This Quote
            </a>
        </div>
    </div>
...
<section class="py-12 bg-gray-50">
    <div class="container mx-auto px-4 text-center">
        <h2 class="text-2xl md:text-3xl font-bold text-amber-900 mb-4">
        ...
        <a href="{{ route('sessions.index') }}"
           class="inline-block px-8 py-4 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition text-lg font-semibold shadow-lg">
